...
# Read in data
rt.data.str<-read_xlsx(path="p207_rtqpcr_20240216_strcomp.xlsx",sheet="Results",col_names = TRUE)
...
rt.data.str[rt.data.str$colID %in% c(1,2,3,4,5,6) & rt.data.str$rowID %in% c("G","H"),]$gene_prim<-"g2600"
rt.data.str[rt.data.str$colID %in% c(7,8,9) & rt.data.str$rowID %in% c("H"),]$gene_prim<-"g2600"

rt.data.str$replicate<-NA
rt.data.str[rt.data.str$colID %in% c(1,4,7) & rt.data.str$rowID %in% c("A","B","C","D","E","F","G","H"),]$replicate<-"1"
rt.data.str[rt.data.str$colID %in% c(2,5,8) & rt.data.str$rowID %in% c("A","B","C","D","E","F","G","H"),]$replicate<-"2"
rt.data.str[rt.data.str$colID %in% c(3,6,9) & rt.data.str$rowID %in% c("A","B","C","D","E","F","G","H"),]$replicate<-"3"
rt.data.str[rt.data.str$colID %in% 10 & rt.data.str$rowID %in% c("A","B","C","D","E","F","G","H"),]$replicate<-"1"
rt.data.str[rt.data.str$colID %in% 11 & rt.data.str$rowID %in% c("A","B","C","D","E","F","G","H"),]$replicate<-"2"
rt.data.str[rt.data.str$colID %in% 12 & rt.data.str$rowID %in% c("A","B","C","D","E","F","G","H"),]$replicate<-"3"
rt.data.str[rt.data.str$colID %in% c(7,10) & rt.data.str$rowID %in% "G",]$replicate<-"2"
rt.data.str[rt.data.str$colID %in% c(8,11) & rt.data.str$rowID %in% "G",]$replicate<-"1"
rt.data.str[rt.data.str$colID %in% 9 & rt.data.str$rowID %in% "G",]$replicate<-"3"
...
rt.data.str[rt.data.str$colID %in% c(4,5,6,7,8,9) & rt.data.str$rowID %in% "G",]$strain<-"clsB"
rt.data.str[rt.data.str$colID %in% c(10,11,12) & rt.data.str$rowID %in% c("G","H"),]$strain<-"clsB"

rt.data.str[rt.data.str$colID %in% c(7,8,9) & rt.data.str$rowID %in% "H",]$strain<-"neg"
rt.data.str[rt.data.str$colID %in% c(4,5,6) & rt.data.str$rowID %in% "H",]$strain<-"empty"
rt.data.str[rt.data.str$colID %in% 3 & rt.data.str$rowID %in% c("D","E","F","G"),]$strain<-"empty"
rt.data.str[rt.data.str$colID %in% 12 & rt.data.str$rowID %in% c("A","B","C"),]$strain<-"empty"
rt.data.str[rt.data.str$colID %in% c(10,11) & rt.data.str$rowID %in% "C",]$strain<-"empty"

rt.data.str$condition<-NA
rt.data.str[rt.data.str$rowID %in% c("A","B","C","D","E","F"),]$condition<-"T1"
rt.data.str[rt.data.str$rowID %in% c("G","H") & rt.data.str$colID %in% c(7,8,9,10,11,12),]$condition<-"T1"
rt.data.str[rt.data.str$rowID %in% c("G","H") & rt.data.str$colID %in% c(1,2,3,4,5,6),]$condition<-"T1_nort"

rt.data.f<-rt.data.str[,c("gene_prim","Well_Position","colID","rowID","CT","strain","replicate","condition")]

rt.data.f$CT<-as.numeric(rt.data.f$CT)
rt.data.f$gene_prim<-factor(rt.data.f$gene_prim)
rt.data.f$condition<-factor(rt.data.f$condition, levels=c("T1","T1_nort"))
...
rt.data.f.agg.nort<-rt.data.f.agg[rt.data.f.agg$condition %in% "T1_nort",]
rt.data.f.agg<-rt.data.f.agg[rt.data.f.agg$condition %!in% "T1_nort",]
...
# ddCt.wt.clsa.p<-ggplot(rt.data_norm.f[!is.na(rt.data_norm.f$delta_delta_Ct_wt.clsa),],aes(x=gene_prim,y=delta_delta_Ct_wt.clsa))+geom_point()+
...
# ddCt.wt.clsb.p<-ggplot(rt.data_norm.f[!is.na(rt.data_norm.f$delta_delta_Ct_wt.clsb),],aes(x=gene_prim,y=delta_delta_Ct_wt.clsb))+geom_point()+
#  as_md_theme(my_theme)+xlab("Gene ID")+ylab("\u0394\u0394 Ct (\u0394clsA Ct - WT Ct)")
# plot(ddCt.wt.clsb.p)
...
fc_gene_early.p<-ggplot(rt.data_norm.f.m[rt.data_norm.f.m$variable %in% c("expression_fold_change_clsa.wt","expression_fold_change_clsb.wt"),])+
  geom_jitter(aes(x=strain_comp,y=log2(value),color=gene_prim))+
  geom_jitter(aes(x=strain_comp,y=log2(value),color=gene_prim))+
...
plot(fc_gene_early.p)

fc_gene_early_bar.p<-ggplot(rt.data_norm.f.m[rt.data_norm.f.m$variable %in% c("expression_fold_change_clsa.wt","expression_fold_change_clsb.wt"),], aes(x=strain_comp,y=log2(value),fill=gene_prim))+
...
plot(fc_gene_early_bar.p)


ggsave(filename="../figures/rtqpcr_cls_straincomp_earlylog_20240216.pdf",plot=fc_gene_early.p,width=1.5,height=2,units="in",device=cairo_pdf)
ggsave(filename="../figures/rtqpcr_cls_straincomp_earlylog_20240216_bar.pdf",plot=fc_gene_early_bar.p,width=1.5,height=2,units="in",device=cairo_pdf)

t.test(rt.data_norm.f[rt.data_norm.f$gene_prim %in% "clsA" & rt.data_norm.f$condition %in% "T1",]$delta_Ct,rt.data_norm.f[rt.data_norm.f$gene_prim %in% "clsA" & rt.data_norm.f$condition %in% "T1",]$delta_Ct_clsB)
# p = 0.1862
t.test(rt.data_norm.f[rt.data_norm.f$gene_prim %in% "clsB" & rt.data_norm.f$condition %in% "T1",]$delta_Ct,rt.data_norm.f[rt.data_norm.f$gene_prim %in% "clsB" & rt.data_norm.f$condition %in% "T1",]$delta_Ct_clsA)
# p = 0.4149
